...
if cffi.__version_info__ < (1, 4, 0):
    raise ImportError("torch.utils.ffi requires cffi version >= 1.4, but " "got " + ".".join(map(str, cffi.__version_info__)))
...
def _make_python_wrapper(name, cffi_wrapper_name, target_dir):
    py_source = PY_MODULE_TEMPLATE.substitute(name=name, cffi_wrapper_name=cffi_wrapper_name)
...
def _wrap_function(function, ffi):
    ...
    def safe_call(*args, **kwargs):
        args = tuple(ffi.cast(_torch_to_cffi.get(arg.type(), "void") + "*", arg._cdata) if isinstance(arg, torch.Tensor) or torch.is_storage(arg) else arg for arg in args)
        args = (function,) + args
